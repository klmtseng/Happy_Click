...
export default function App() {
  ...
  const [feedbacks, setFeedbacks] = useState<{id: number, text: string, x: number, y: number, rotate: number, color: string}[]>([]);
  ...
  useEffect(() => {
    ...
    // Calculate rank: Count how many scores in leaderboard are strictly greater than current score
    const betterScores = leaderboard.filter(entry => entry.score > score).length;
    const newRank = betterScores + 1;

    if (newRank < prevRank) {
        // Rank Improved!
        setShowRankUp(true);
        playSpecialVoice("RANK UP!", true); // Echo ON
  ...
  }, [score, prevRank, currentRank, playSpecialVoice]);

  // Game Over Logic
  useEffect(() => {
    // If combo drops to 0 AND we have a score (score > 0) AND not already over
  ...
  useEffect(() => {
    ...
    if (isHyperspace && combo >= hyperspaceTarget + 5) {
        ...
        // Change Star Color (Random Rainbow)
        ...
        // Set Next Target (Strictly every 300)
        setHyperspaceTarget(t => t + 300);
    }
  }, [combo, hyperspaceTarget, isHyperspace, playHyperspaceSound, starColor]);

  // Handle Feedback Spawning
  useEffect(() => {
    if (combo > 0 && combo % 20 === 0) {
        // Expanded pool of words
        const texts = [
            "COOL!", "SICK!", "RAD!", "EPIC!", "WILD!", "HYPER!", "MAX!", "YEAH!", 
            "SUPER!", "INSANE!", "LEGEND!", "GODLIKE!", "POWER!", "SPEED!", "CRAZY!"
        ];
        
        // Only force "UNSTOPPABLE" very rarely or just mix it in
        // The previous logic forced it for ALL combos > 100, which is why it repeated.
        // Let's make it random even at high combos, but include "UNSTOPPABLE" in the pool.
        
        let pool = texts;
        if (combo >= 50) pool = [...pool, "AMAZING!", "AWESOME!"];
        if (combo >= 100) pool = [...pool, "UNSTOPPABLE!", "MONSTER!", "GODLIKE!"];

        // Filter out the last one to avoid immediate repeat
        let candidates = pool.filter(t => t !== lastFeedbackTextRef.current);
        // Fallback if filter leaves nothing (shouldn't happen with this many words)
        ...
        // Random position (avoiding center where button is)
        // Button is roughly center. Let's spawn in the outer 2/3rds
        let x = Math.random() * 80 + 10; // 10% to 90%
        let y = Math.random() * 80 + 10;
        
        // Simple check to push away from center (approx 50,50)
        ...
        const newFeedback = {
            ...
            x,
            y,
            rotate: Math.random() * 40 - 20, // -20 to 20 deg
            color: colors[Math.floor(Math.random() * colors.length)]
        };

        setFeedbacks(prev => [...prev, newFeedback]);
        
        // Play louder sound + Echo Voice
        playEncouragementSound();
        playSpecialVoice(text, false); // Echo OFF
  ...
  }, [combo, playEncouragementSound, playSpecialVoice]);

  // Handle Decay Sound
  useEffect(() => {
    if (isDecaying) {
        playDecaySound(combo);
    }
  }, [isDecaying, playDecaySound, combo]); // Trigger on every combo drop
  ...
  useEffect(() => {
    if (combo === 67) {
      ...
      if (customSfxUrl) {
        ...
        // Fallback to TTS
        playSpecialVoice("SIX SEVEN!!", true); // Echo ON
      }
      
      setTimeout(() => setShowSpecial67(false), 800);
    }
  }, [combo, playSpecialVoice, customSfxUrl]);
  ...
  const shakeAmount = Math.max(0, (level - 2) * 2);
  ...
  return (
    <div 
        ...
        onClick={initAudio} // Ensure audio context starts on first interaction anywhere
    >
      <BackgroundEffects 
        level={level} 
        combo={combo} 
      ...
      />
      {/* <Leaderboard currentScore={maxCombo} /> */}
      ...
      {/* Main Content Container with Shake Effect */}
      <motion.div
        animate={
            isChaos 
      ...
      <AnimatePresence>
        {showSpecial67 && (
          <div className="fixed inset-0 flex items-center justify-center z-50 pointer-events-none">
            ...
            {/* Firework Particles (Simulated with multiple divs) */}
            {[...Array(12)].map((_, i) => (
               <motion.div
                  key={i}
